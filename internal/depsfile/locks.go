...
type ProviderLock struct {
	...
	// which we allow as a stop-gap until we can upgrade Terraform Registry
	// to support the new scheme but is non-ideal because we can verify it only
	// when we have the original .zip file exactly, and so we can't verify
	// a local directory containing the unpacked contents of that .zip file.
	//
